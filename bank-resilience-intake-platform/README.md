...
本版已完成 Demo / POC 所需的治理展示能力，包含規則測試、瀏覽器驗證、RWD 檢查、列印版式與 Evidence Pack 完整性指紋。

- 資料儲存：採 localStorage，定位為 Demo 假資料環境，不存放任何真實公司或個人資料
- 風險模型：採可解釋規則模型，所有判斷均列出觸發原因與政策來源；正式導入時可再接企業風險量化方法
- PDF：採 print-friendly HTML，支援 A4 列印與瀏覽器存成 PDF，適合 POC 會議附件
- 稽核證據：Evidence Pack 已包含證據包 ID、資料版本、規則版本、匯出時間與 Demo 完整性指紋
- 權限控管：本版聚焦前期治理流程展示；正式導入時可接登入、角色權限、多人協作與正式稽核紀錄
- 驗收指令：`npm test` 會執行 TypeScript 檢查、規則品質檢查與 production build
- 瀏覽器驗證：已用 in-app browser 檢查 Dashboard、Executive Storyboard、Evidence Pack、Compliance Lineage、Cross-functional Tasks 與手機版 Evidence Pack
