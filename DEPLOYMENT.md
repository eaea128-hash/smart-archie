...
```bash
# 1. Clone the repo
git clone https://github.com/YOUR_ORG/smart-archie.git
cd smart-archie
...
### Branch Strategy
```
main        → Production (smartarchie.ai)
...
├── js/
│   ├── utils.js            # Toast, Store, formatting
│   ├── auth.js             # LocalStorage-based auth
│   ├── analyze-engine.js   # Rule-based fallback engine
│   └── api-client.js       # Claude API client (with fallback)
├── netlify/
│   └── functions/
│       ├── analyze.js      # Claude API → analysis
│       └── trends.js       # International trends data
├── .github/workflows/
│   ├── ci.yml              # PR checks
...
Running Smart Archie with real Claude API:
- Each analysis: ~3,000–8,000 input tokens + ~6,000–8,000 output tokens
