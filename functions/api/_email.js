 ...
 * 此為內部 helper，不是 Cloudflare Pages Function endpoint
 *
 * 需要環境變數：RESEND_API_KEY
 * 函式接受 env 參數以支援 Cloudflare Workers 環境
 */
...
export async function sendEmail({ to, subject, html }, env) {
  ...
  const from = env?.EMAIL_FROM || 'CloudFrame <onboarding@resend.dev>';
...
// ── 快捷方式 ──────────────────────────────────────────────────
export async function sendWelcomeEmail({ to, name }, env) {
  return sendEmail({
    to,
...
export async function sendQuotaWarningEmail({ to, name, used, limit, plan }, env) {
  return sendEmail({
    to,
